...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash('Вхід не виконано. Перевірте будь ласка свій емейл і пароль', 'danger')
...
def account():
    ...
    return render_template("account.html", title="Профіль", image_file=image_file,
                           form=form)
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash("Ваш пост був успішно створений!", 'success')
        return redirect(url_for('home'))
    return render_template("create_post.html", title="Новий пост", form=form)
